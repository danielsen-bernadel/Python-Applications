...
def get_dates():
    ...
    end = datetime.now(timezone) + dt.timedelta(days=3)
    st_year = start.year
    st_month = inttoStrMonth(int(start.month))
    if start.day < 10:
        st_day = "0"+str(start.day)
    else:
        st_day = start.day
    global start_date
    start_date = "calendar?day="+str(st_month)+str(st_day)+"."+str(st_year)

    end_year = end.year
    end_month = inttoStrMonth(int(end.month))
    if end.day < 10:
        end_day = "0"+str(end.day)
    else:
        end_day = end.day
    global end_date
    end_date = "calendar?day="+str(end_month)+str(end_day)+"."+str(end_year)
...
if __name__ == "__main__":
	...
	get_dates()
	getEventsCalendar( start_date, end_date, file_path)